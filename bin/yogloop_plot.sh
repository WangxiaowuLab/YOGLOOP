#!/bin/bash
# yogloop_label.sh - Plot heatmaps for loop label and identify

# Display usage information
usage() {
    echo "Usage: $0 -d <interaction_dir> -g <GTF/GFF3> -b <background file> [-w window_size] [-s step_size] [-r heatmap_region] [-t threads]"
    echo "Options:"
    echo "  -d  Directory containing interaction files (required)"
    echo "  -g  Genome annotation file (required)"
...
while getopts ":d:g:b:w:s:r:t:" opt; do
    case $opt in
...
if [ -z "$INTERACTION_DIR" ] || [ -z "$ANNO_FILE" ] || [ -z "$BACKGROUND" ]; then
...
if [ ! -f "$ANNO_FILE" ]; then
    echo "Error: Genome annotation file $ANNO_FILE not found" >&2
    exit 1
fi

# Create output directory if it doesn't exist
awk '{print $1}' $ANNO_FILE | uniq | while read c;
    do
...
produce_heatmap() {
    ...
    #awk -v w=$WINDOW_SIZE -v s=$STEP_SIZE -v r=$HEATMAP_REGION -v d=$INTERACTION_DIR -v o=$OUTPUT_DIR -v b=$BACKGROUND -v st=$standard '{for(i=0;i<$2;i+=10000){print $1,i,i+r,w,s,d,o,b,st}}' $ANNO_FILE | xargs -n 1 -I {} -P $THREADS sh -c "sh $SCRIPT_DIR/label_slide.sh {}"
    awk -v w=$WINDOW_SIZE -v s=$STEP_SIZE -v r=$HEATMAP_REGION -v d=$INTERACTION_DIR -v o=$OUTPUT_DIR -v b=$BACKGROUND -v st=$standard '$3~/gene/{mid=int(($4+$5)/2);print $1,mid-r/2,mid+r/2,w,s,d,o,b,st}' $ANNO_FILE | xargs -n 1 -I {} -P $THREADS sh -c "sh $SCRIPT_DIR/label_slide.sh {}"
