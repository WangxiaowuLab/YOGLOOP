#!/bin/bash
# yogloop_bg.sh - Background analysis module for Yoloop software

# Display usage information
usage() {
    echo "Usage: $0 -d <interaction_dir> -g <genome_size> [-w window_size] [-s step_size] [-r heatmap_region] [-t threads]"
    echo "Options:"
...
while getopts ":d:g:w:s:r:t:" opt; do
    case $opt in
...
analyze_background() {
    ...
    #awk -v w=$WINDOW_SIZE -v s=$STEP_SIZE -v d=$INTERACTION_DIR -v o=$OUTPUT_DIR r=$HEATMAP_REGION '$3~/gene/{mid=int(($4+$5)/2);print $1,mid-r/2,mid+r/2,w,s,d,o}' $GENOME_SIZE | shuf -n 500 | xargs -n 1 -I {} -P $THREADS sh -c "sh $SCRIPT_DIR/bg_slide.sh {}"
    ...
    rm ${HEATMAP_REGION}_background.txt && mv ${HEATMAP_REGION}_long.txt ${HEATMAP_REGION}_background.txt
