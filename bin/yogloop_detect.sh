...
# Usage: ./yogloop_detect.sh -m <model> -c <class> -i <input_dir> -t [conf] -w [window]
...
while getopts ":m:c:i:t:w:" opt; do
  case $opt in
...
if [ -z "$MODEL" ] || [ -z "$CLASS_FILE" ] || [ -z "$INPUT_DIR" ] ; then
    ...
    echo "  -w  Heatmap size (default:20000)"
    exit 1
fi
...
sh ${SCRIPT_DIR}/picture2genome.sh $OUTPUT_DIR $CONF $WINDOW
